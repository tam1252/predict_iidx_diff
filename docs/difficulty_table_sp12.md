# ☆12 SP 非公式難易度表

BPI・CPI データをもとにモデルで算出した非公式難易度です。

- レベル範囲: **11.5 〜 13.0**（0.1 刻み）
- 個人差: **高** / **中** / **低** / **—**（データなし）
- `*` マークはモデル予測のみ（BPI/CPI 実データなし）

---

## 13.0  （30 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| DIAVOLO | 低 |
| Dans la nuit de l'éternité | 低 |
| Fascination MAXX | 低 |
| Level 5 | 低 |
| Mare Nectaris | 低 |
| RAGE feat.H14 of LEONAIR | 低 |
| Sinus Iridum | 低 |
| Skratch Education Lv-1 | 低 |
| Somnidiscotheque | 低 |
| SμG@R RU$# | 低 |
| Thor's Hammer | 低 |
| XHRONOXAPSULΞ | 低 |
| Y&Co. is dead or alive | 低 |
| n/a | 低 |
| ディスコルディア | 低 |
| ピアノ独奏無言歌 \ | 低 |
| 冥 | 低 |
| 卑弥呼 | 低 |
| 惑星鉄道 | 低 |
| 灼熱 Pt.2 Long Train Running | 低 |
| 灼熱Beach Side Bunny | 低 |
| 駅猫のワルツ | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| 27th style * | — |
| CODE:Ø * | — |
| ICARUS† | 低 |
| Illegal Function Call | 低 |
| Miracle 5ympho X | 低 |
| SAMURAI-Scramble | 低 |
| Sigmund†LEGGENDARIA | 低 |
| 疾風迅雷†LEGGENDARIA | 低 |

## 12.9  （8 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| Go Beyond!! | 低 |
| Inferno of Fomalhaut | 低 |
| Level 4 | 低 |
| Makin' It * | — |
| Plan 8 | 低 |
| THE PEERLESS UNDER HEAVEN | 低 |
| VØID | 低 |
| 二人ノ廃城幽踊宴 | 低 |

## 12.8  （16 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| 3y3s | 低 |
| Carmina | 低 |
| DAY DREAM | 低 |
| Glitch N Ride | 低 |
| Go Ahead!! | 低 |
| Ignis†Iræ | 低 |
| Level One | 低 |
| Red. by Jack Trance | 低 |
| Session 12-Esther- * | — |
| The Chase | 低 |
| perditus†paradisus | 低 |
| Χ-DEN * | — |
| 恋愛=精度×認識力 | 低 |
| 音楽 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| KAMAITACHI | 低 |
| カジノファイヤーことみちゃん * | — |

## 12.7  （18 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| Everlasting Message | 低 |
| FiZZλ_PØT!OИ * | — |
| LOCUS OF THE TRAVEL | 低 |
| Level 3 | 低 |
| Sigmund | 低 |
| The Clown of 24stairs | 低 |
| fixer * | — |
| 天使のカンタータ -Cantata of Angels- | 低 |
| 焔極OVERKILL | 低 |
| 華麗なる大犬円舞曲 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| #The_Relentless | 低 |
| ChaserXX | 低 |
| EBONY & IVORY† | 低 |
| SEITEN NO TERIYAKI * | — |
| TRANOID | 低 |
| Time to Air | 低 |
| VANESSA† | 低 |
| Verflucht†LEGGENDARIA | 低 |

## 12.6  （30 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| 199024club -Re:BounceKiller- | 低 |
| Ambivalent Vermilia | 低 |
| Amor∞Fati * | — |
| BLUE MIRAGE | 低 |
| Bad Encryption | 低 |
| DUE TOMORROW | 低 |
| Elemental Creation | 低 |
| GuNGNiR | 低 |
| HEROES * | — |
| ICARUS | 低 |
| IX | 低 |
| JOMANDA | 低 |
| Snake Stick | 低 |
| TOMAHAWK | 低 |
| Timepiece phase II(CN Ver.) | 低 |
| VOX UP | 低 |
| voltississimo | 低 |
| シムルグの目醒め | 低 |
| ピアノ協奏曲第１番\ | 低 |
| 東京神話 | 低 |
| 栄冠のカンパネラ | 低 |
| 焱影 * | — |

### SPL

| 曲名 | 個人差 |
|------|--------|
| Be quiet | 低 |
| Beat Juggling Mix * | — |
| KAISER PHOENIX† | 低 |
| MAD ATTACK | 低 |
| REVOLVER | 低 |
| four pieces of heaven | 低 |
| お米の美味しい炊き方、そしてお米を食べることによるその効果。 | 低 |
| 君のハートにロックオン | 低 |

## 12.5  （27 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| , * | — |
| 3!dolon Forc3 * | — |
| Binary Black Hole | 低 |
| Blue Spring Express | 低 |
| COSMIC RAY | 低 |
| Chrono Diver -PENDULUMs- | 低 |
| Confiserie | 低 |
| Cult Invitation | 低 |
| MEGAERA | 低 |
| Purple Perplex | 低 |
| SCREW // owo // SCREW | 低 |
| Smashing Wedge | 低 |
| The Rebellion of Sequencer | 低 |
| Unbelief | 低 |
| ra'am | 低 |
| rage against usual | 低 |
| スキッテイエェエエェ!!!! | 低 |
| 色を喪った街 * | — |
| 金野火織の金色提言 | 低 |
| 魔法のかくれんぼ | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| Do it!! Do it!! * | — |
| Feel The Beat†LEGGENDARIA | 低 |
| Ganymede | 低 |
| Phoenix | 低 |
| Saturn | 低 |
| THE CANNONBALLER * | — |
| invoker†LEGGENDARIA | 低 |

## 12.4  （43 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| 2 Beasts Unchained | 低 |
| ASIAN VIRTUAL REALITIES (MELTING TOGETHER IN DAZZLING DARKNESS) | 低 |
| Almagest | 低 |
| Artist | 低 |
| Cross Fire | 低 |
| DropZ-Line- | 低 |
| GAMBOL * | — |
| GO OVER WITH GLARE -ROOTAGE 26- | 低 |
| KAISER PHOENIX | 低 |
| Memoria Obscura * | — |
| Mächö Mönky | 低 |
| PARADISE LOST | 低 |
| PARANOiA ～HADES～ | 低 |
| Peaktime Booster | 低 |
| Reflux | 低 |
| SISYPHUS | 低 |
| Skreaming for Salvation * | — |
| StrayedCatz | 低 |
| Watch Out Pt.2 | 低 |
| Zirkfied | 低 |
| reunion | 低 |
| ruin of opals | 低 |
| キャトられ♥恋はモ～モク * | — |
| 共鳴遊戯の華 | 低 |
| 子供の落書き帳 | 低 |
| 恋する☆宇宙戦争っ!! | 低 |
| 桜 | 低 |
| 水鏡の異界 | 低 |
| 烽火連天の刃 | 低 |
| 煉獄のエルフェリア | 低 |
| 聖人の塔 | 低 |
| 魅惑のYUMMYスイーツ | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| Ancient Scapes†LEGGENDARIA | 低 |
| BroGamer | 低 |
| CONTRACT | 低 |
| HYENA | 低 |
| Lost Souls | 低 |
| NEW GENERATION-もう、お前しか見えない- | 低 |
| POLꓘAMAИIA * | — |
| STEEL NEEDLE | 低 |
| Violet Pulse | 低 |
| 廿 | 低 |
| 超青少年ノ為ノ超多幸ナ超古典的超舞曲 | 低 |

## 12.3  （41 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| "[ * | — |
| -65℃ | 低 |
| 24h Endurance Race | 低 |
| 255 | 低 |
| BLACK.by X-Cross Fade | 低 |
| CADENZA | 低 |
| DORNWALD ～Junge～ | 低 |
| Devilz Staircase | 低 |
| Empire of Fury | 低 |
| HADES | 低 |
| HAERETICUS | 低 |
| IMPLANTATION * | — |
| INSOMNIA | 低 |
| Level 2 | 低 |
| MAX 300 | 低 |
| Monopole. | 低 |
| OTENAMI Hurricane | 低 |
| Pinky Happy Crazy * | — |
| Push on Beats!～音ゲの国のeX-ストリーマー～ | 低 |
| RIZING-GAMERS. * | — |
| STULTI | 低 |
| Scripted Connection⇒ * | — |
| TECHNOPHOBIA | 低 |
| THE F∀UST | 低 |
| Timepiece phase II | 低 |
| UNDO THE NIGHT * | — |
| Uaigh Gealaí | 低 |
| Xlø * | — |
| kors k's How to make OTOGE CORE 「LONG」 | 低 |
| ⁽⁽ଘ( ˙꒳˙ )ଓ⁾⁾ beyond reason * | — |
| がっつり陰キャ!?怪盗いいんちょの億劫^^; | 低 |
| ワルツ第17番 ト短調\ | 低 |
| 嘆きの樹 | 低 |
| 灼熱 Lost Summer Dayz | 低 |
| 轟け！恋のビーンボール！！ | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| B4U(BEMANI FOR YOU MIX) | 低 |
| Caldwell 99 * | — |
| Close the World feat.a☆ru†LEGGENDARIA | 低 |
| Override | 低 |
| SOLID WYVERN | 低 |
| Secrets | 低 |

## 12.2  （60 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| "Friction[! * | — |
| Angel's Ladder | 低 |
| BLACK or WHITE? | 低 |
| BLUE DRAGON(雷龍RemixIIDX) | 低 |
| Caterpillar | 低 |
| Concertino in Blue | 低 |
| DEADHEAT | 低 |
| Dances with Snow Fairies | 低 |
| Devil's Gear | 低 |
| Dynamite | 低 |
| Flashes | 低 |
| Flämingo * | — |
| Grand Chariot | 低 |
| I | 低 |
| Innocent Walls | 低 |
| Invitation from Mr.C | 低 |
| LASER CRUSTER (IGNITE REMIX) | 低 |
| LOST TECHNOLOGIE | 低 |
| MAXIMUM CHEAT GIRL | 低 |
| NITROUS CANNON | 低 |
| New Castle Legions | 低 |
| ORCA | 低 |
| One for All | 低 |
| Out of Control | 低 |
| PLEASE DON'T GO | 低 |
| Papilio ulysses | 低 |
| Red. by Full Metal Jacket | 低 |
| Satellite Burst | 低 |
| Session 9-Chronicles- | 低 |
| Trill auf G | 低 |
| VOLAQUAS | 低 |
| Vitrum | 低 |
| Without Heaven | 低 |
| Xb10r | 低 |
| eRAseRmOToRpHAntOM | 低 |
| hora de verdad | 低 |
| quell～the seventh slave～ | 低 |
| suspicions | 低 |
| tripping contact * | — |
| †渚の小悪魔ラヴリィ～レイディオ†(IIDX EDIT) | 低 |
| アストライアの双皿 | 低 |
| ピアノ協奏曲第１番\ (BlackY Remix) | 低 |
| 世界の果てに約束の凱歌を -ReUnion- | 低 |
| 今宵、ロマンス横丁。 | 低 |
| 天空の夜明け | 低 |
| 少年は空を辿る | 低 |
| 最小三倍完全数 | 低 |
| 雪上断火 | 低 |
| 電腦都市 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| AIR RAID FROM THA UNDAGROUND | 低 |
| ALBA-黎明- * | — |
| Anisakis-somatic mutation type \- | 低 |
| Bow shock!! | 低 |
| CoMAAAAAAA * | — |
| G2 * | — |
| O/D*20 | 低 |
| THANK YOU FOR PLAYING | 低 |
| Twelfth Style | 低 |
| ZEPHYRANTHES | 低 |
| 俺ら東京さ行ぐだ * | — |

## 12.1  （61 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| 1116 | 低 |
| ALTERNATOR | 低 |
| AO-1 | 低 |
| Bad Maniacs | 低 |
| Boomy and The Boost | 低 |
| Drop Syndicate | 低 |
| FAKE TIME | 低 |
| FANTASTIC THREE | 低 |
| Fire Beat | 低 |
| G59 | 低 |
| GAME ON | 低 |
| Hollywood Galaxy | 低 |
| Illusionary Waterlily | 低 |
| Immortal * | — |
| MENDES | 低 |
| Million Dollar | 低 |
| NOIXIA | 低 |
| One More Lovely | 低 |
| PERFECT GREAT!! * | — |
| Painful Fate | 低 |
| Proof of the existence | 低 |
| Regina vespaE * | — |
| SCREAM SQUAD | 低 |
| Sense 2007 | 低 |
| Silver Bullet | 低 |
| Slipstream | 低 |
| Sol Cosine Job 2 | 低 |
| THE BRAVE MUST DIE | 低 |
| THE DAY OF JUBILATIONS | 低 |
| TRIUMPH | 低 |
| The Least 100sec | 低 |
| Tiempo Loco | 低 |
| Triple Counter | 低 |
| VOX RUSH * | — |
| Verflucht | 低 |
| Xperanza | 低 |
| YAKSHA | 低 |
| chaplet -IIDX re:build- * | — |
| dica dica | 低 |
| fallen leaves -IIDX edition- | 低 |
| fffff | 低 |
| innocent revolver | 低 |
| one or eight | 低 |
| ιgniЯRuina * | — |
| ∀ | 低 |
| ぞうしょく！？マイデンティティ | 低 |
| アラビアン ハイパー ナイト | 低 |
| 伐折羅-vajra- | 低 |
| 少年A | 低 |
| 未完成ノ蒸氣驅動乙女 | 低 |
| 灼熱Beach Side Bunny(かめりあ's \ Remix) | 低 |
| 疾風迅雷 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| ALL MY TURN-このターンに、オレの全てを賭ける- * | — |
| Arabian Rave Night | 低 |
| Burning Flame | 低 |
| CHRONO DIVER -NORNIR- | 低 |
| INAZUMA * | — |
| Marie Antoinette | 低 |
| STARLIGHT DANCEHALL | 低 |
| Submerge Serenade * | — |
| 神っぽいな * | — |

## 12.0  （62 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| 2 Be Continued * | — |
| ANCHOR | 低 |
| Arkadia | 低 |
| BEAT PRISONER | 低 |
| BIGWAVERS | 低 |
| BLAZING_LAZER | 低 |
| Begin * | — |
| Dahlia | 低 |
| Despair of ELFERIA | 低 |
| Disco Killer Music Lover * | — |
| EVANESCENT | 低 |
| EXUSIA | 低 |
| EYE OF THE HEAVEN | 低 |
| Evans | 低 |
| Flying Castle | 低 |
| HyperTwist * | — |
| Konzert V | 低 |
| Last Dance | 低 |
| Liberation | 低 |
| Life Is A Game ft.DD\Metal | 低 |
| NEO GENERATOR SEVEN | 低 |
| Pārvatī | 低 |
| Rave*it!! Rave*it!! | 低 |
| Say YEEEAHH | 低 |
| Scripted Connection⇒ A mix * | — |
| Smalt #28598F | 低 |
| Super Freak | 低 |
| TOXIC VIBRATION | 低 |
| The Last Apocalypse | 低 |
| The Limbo | 低 |
| V * | — |
| Vermilion Carol | 低 |
| We're so Happy(P*Light Remix) IIDX ver. | 低 |
| X | 低 |
| cinder | 低 |
| rommmo | 低 |
| uәn * | — |
| Übertreffen† * | — |
| Ōu Legends | 低 |
| トリカゴノ鳳凰 | 低 |
| ビビッド ☆＋＊。キラキライム * | — |
| 即席！脳直★ミュージックシステム | 低 |
| 夏色DIARY - L.E.D.-G STYLE MIX - | 低 |
| 抱きしめてモナムール | 低 |
| 津軽雪 | 低 |
| 深淵に捧ぐレクイエム | 低 |
| 電光 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| Amazing Mirage | 低 |
| Blue Rain | 低 |
| GUILTY | 低 |
| Like+it! * | — |
| MENTAL MELTDOWN * | — |
| QUANTUM TELEPORTATION | 低 |
| Scripted Connection⇒ H mix * | — |
| Super Duper Racers * | — |
| THE DEEP STRIKER | 低 |
| yellow head joe | 低 |
| Übertreffen * | — |
| ハイテックトキオ | 低 |
| ミュージック・アワー * | — |
| ミラージュ･レジデンス | 低 |
| ユミル * | — |

## 11.9  （61 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| ACTØ | 低 |
| AKASHIC BREAK | 低 |
| Always We Trust In You | 低 |
| B.O.D.Y. | 低 |
| CODE:1 | 低 |
| Catch Our Fire! | 低 |
| Colors(radio edit) | 低 |
| DOMINION | 低 |
| EBONY & IVORY | 低 |
| Feel The Beat | 低 |
| Fegrix | 低 |
| FlyAway | 低 |
| From Human Blood * | — |
| GRAVITON | 低 |
| Gravigazer | 低 |
| KILL EACH OTHER | 低 |
| King of Tribe * | — |
| L.E.D. & HuΣeR\, * | — |
| L.F.O | 低 |
| Lethal Weapon | 低 |
| Lightspeed | 低 |
| Ludus In Tenebris * | — |
| MARLIN | 低 |
| Meissa | 低 |
| Mid-Corner (Magic Maker) | 低 |
| ONE AND ONLY | 低 |
| PLASMA SOUL NIGHT feat. Nana Takahashi / 709sec. | 低 |
| POSSESSION | 低 |
| Persephone | 低 |
| Please Welcome Mr.C * | — |
| RINИE * | — |
| SAY BAY | 低 |
| SOLID STATE SQUAD -RISEN RELIC REMIX- | 低 |
| Seven Times Four | 低 |
| Sky High | 低 |
| T-REX vs Velociraptor (In the Far east euphoria) | 低 |
| TECHNO Style Essentials | 低 |
| VANESSA | 低 |
| Vinculum stellarum | 低 |
| anthracene * | — |
| neu | 低 |
| u gotta beat | 低 |
| クレッシェンド * | — |
| 儚キ戀ノ幻想譚 | 低 |
| 八戸亀生羅\, * | — |
| 海神 | 低 |
| 狂イ咲ケ焔ノ華 | 低 |
| 神謳 -RESONANCE- | 低 |
| 禊 | 低 |
| 童話回廊 | 低 |
| 紫陽花 -AZISAI- | 低 |
| 終焔のClaudia | 低 |
| 電人、暁に斃れる。 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| CaptivAte2～覚醒～ * | — |
| Cosmic Cat | 低 |
| INHERITANCE * | — |
| Kung-fu Empire | 低 |
| garden * | — |
| ÆTHER * | — |
| 龍と少女とデコヒーレンス | 低 |
| 龍王の霊廟(Mausoleum Of The Primal Dragon) | 低 |

## 11.8  （43 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| AA -rebuild- | 低 |
| Ancient Scapes | 低 |
| Arca | 低 |
| Astra Blaze * | — |
| BITTER CHOCOLATE STRIKER | 低 |
| COSMIC V3LOCITY | 低 |
| Chaotoxin | 低 |
| Close the World feat.a☆ru | 低 |
| Colorful Cookie | 低 |
| DARK LEGACY | 低 |
| DIVINE JUDGEMENT | 低 |
| DRUNK MONKY * | — |
| FIRE FIRE | 低 |
| FUZIN RIZIN | 低 |
| Hollywood Galaxy(DJ NAGAI Remix) * | — |
| LASER CRUSTER | 低 |
| Lisa-RICCIA * | — |
| Little Star | 低 |
| Matt Silver | 低 |
| No Border | 低 |
| RALLY '25 * | — |
| SYNC-ANTHEM | 低 |
| Shooting Fireball | 低 |
| Sounds Of Summer | 低 |
| Space Battleship S4TØ * | — |
| Triple Cross | 低 |
| Viridian | 低 |
| gigadelic | 低 |
| refrain | 低 |
| 〆 | 低 |
| それは花火のような恋 | 低 |
| たまゆら | 低 |
| イザナミノナゲキ | 低 |
| シュレーディンガーの猫 | 低 |
| リリーゼと炎龍レーヴァテイン | 低 |
| 不沈艦CANDY | 低 |
| 九尾狐夜行 | 低 |
| 旋律のドグマ～Misérables～ | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| M.D.Injection * | — |
| spiral galaxy | 低 |
| waxing and wanding | 低 |
| デモーニッシュ * | — |
| 新宝島 * | — |

## 11.7  （49 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| #CMFLG | 低 |
| #MAGiCVLGiRL_TRVP_B3VTZ | 低 |
| A | 低 |
| ALBIDA | 低 |
| Adularia | 低 |
| Bring The Fire | 低 |
| Double Dribble | 低 |
| ELECTRIC MASSIVE DIVER * | — |
| EX-MASSIVE FIGHTER | 低 |
| GAIA | 低 |
| GENE | 低 |
| GOLDEN CROSS | 低 |
| Ghost Pulse | 低 |
| HARD BRAIN | 低 |
| Hat Surprise (Season 2) | 低 |
| Here We Go Now | 低 |
| Idola | 低 |
| LIVE DRIVING!! feat. 花たん | 低 |
| M4K3 1T B0UNC3 | 低 |
| Megalara Garuda | 低 |
| NNRT | 低 |
| PUNISHER | 低 |
| RAIN | 低 |
| Rave Cannon | 低 |
| Roar of Chronos | 低 |
| SpaceLand☆TOYBOX | 低 |
| Sword of Vengeance | 低 |
| The Sampling Paradise | 低 |
| Thunderbolt | 低 |
| Valanga | 低 |
| Visterhv | 低 |
| dAuntl3ss | 低 |
| destination | 低 |
| encounter | 低 |
| frequent | 低 |
| moon_child | 低 |
| quaver♪ | 低 |
| めいさいアイドル☆あいむちゃん♪ | 低 |
| 天邪鬼 | 低 |
| 表裏一体!?怪盗いいんちょの悩み♥ * | — |
| 雪月花 | 低 |
| 黒髪乱れし修羅となりて | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| GRID KNIGHT * | — |
| Little Little Princess | 低 |
| Ypsilon | 低 |
| thunder HOUSE NATION Remix * | — |
| ちょえちょえまぎか * | — |
| 仮想空間の旅人たち | 低 |
| 華爛漫-Flowers- * | — |

## 11.6  （24 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| Aqvion | 低 |
| Broken Sword | 低 |
| Candy Galy | 低 |
| Carry Me Away | 低 |
| Dr. Chemical & Killing Machine | 低 |
| F | 低 |
| NINJA IS DEAD IIDX ver. | 低 |
| Nightmare before oversleep | 低 |
| Sirius | 低 |
| Stereo Beasts | 低 |
| Summerlights(IIDX Edition) | 低 |
| TROOPERS | 低 |
| The Onlyonez | 低 |
| Venom | 低 |
| WONDER WALKER | 低 |
| entelecheia | 低 |
| invoker | 低 |
| oratio | 低 |
| カゴノトリ～弐式～ * | — |
| 刃図羅 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| CaptivAte～浄化～ * | — |
| Nothing but Theory * | — |
| Plazma * | — |
| RED ZONE | 低 |

## 11.5  （38 曲）

### SPA

| 曲名 | 個人差 |
|------|--------|
| AA | 低 |
| Apocalypse | 低 |
| BIGソムタム | 低 |
| Broken | 低 |
| Drastic Dramatic | 低 |
| JUSTICE/GUILTY feat. Nana Takahashi & 709sec. | 低 |
| LIGHTNING STRIKES | 低 |
| Primitive Vibes | 低 |
| Rampage | 低 |
| Raspberry Railgun | 低 |
| Rave Patroller | 低 |
| Ristaccia | 低 |
| SPECIAL SUMMER CAMPAIGN! | 低 |
| Super Rush | 低 |
| Symmetry | 低 |
| TIEFSEE | 低 |
| Todestrieb | 低 |
| True Blue | 低 |
| ZZ | 低 |
| glacia | 低 |
| mosaic * | — |
| sakura storm | 低 |
| ΕΛΠΙΣ | 低 |
| 《PL|RAYER》 | 低 |
| スパークリング☆彡ハイパーチューン！！ | 低 |
| 千年ノ理 | 低 |
| 残像ニ繋ガレタ追憶ノHIDEAWAY | 低 |
| 紅牡丹 | 低 |

### SPL

| 曲名 | 個人差 |
|------|--------|
| A MINSTREL ～ ver.short-scape ～ | 低 |
| Beat Radiance | 低 |
| CUE CUE RESCUE | 低 |
| KING | 低 |
| Sweet Clapper * | — |
| snow storm | 低 |
| グッバイ宣言 * | — |
| セピアの軌跡 ft. 天宮みや(少女フラクタル) * | — |
| フォニイ * | — |
| 華蝶風雪 | 低 |

