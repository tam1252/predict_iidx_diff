...
class TextageDecoder:
    ...
    def decode_cn(self, cn, side=1):
        ...
        for measure, entries in cn[side].items():
            ...
            for entry in entries:
                ...
                pos = start * 3
                # 始端と終端だけでなく、ノーツとしてカウントされるフラグ
                # 1 = この小節で開始、3 = 単一小節内で完結、7 = MSS
                if flag in (1, 3, 7):
    ...
    def deduplicate_notes(self, normal_dict):
        """ JSの `objtab[v2h] |= (1<<ob2)` による同一pos/同一laneのノーツ重複排除をエミュレートする """
        total_notes = 0
        for measure, notes in normal_dict.items():
            # cn_endはカウント対象外なので削除する
            filtered_notes = [n for n in notes if n.get("type") != "cn_end"]
            
            # setによる重複排除 (pos と lane の組み合わせ)
            unique_notes = set()
    ...
    def parse_url(self, url):
        ...
        for i, p in enumerate(raw_parts):
            p = p.strip()
            measure_num = i + 1
            ln_n = measure_lens.get(i, 384) # i is the JS index
...
if __name__ == "__main__":
    ...
    if data:
        # 重複排除付きでカウント
        unique_cnt = decoder.deduplicate_notes(data["normal"])
        ...
        print(f"合計ノーツ数 (重複排除後): {unique_cnt}  (公式: 2401)")
        print(f"合計ノーツ数 (素): {total}")
