...
class TextageDecoder:
    ...
    def decode_hash_format(self, sdd, ln_n=384):
        ...
        # JS側では小節またぎで `v2c` が維持される可能性があるため self_v2c を使用
        # if(sdd.charAt(0)=="#"){ ... v2c=0; ... } という処理がJSの先頭にあるため
        # 実際にはハッシュが来たら初期化される。
        self.v2c = 0 
        
        while sft < len(sdd):
            v2o  = ""
            v2v  = (1 if self.v2c else 3) * ln_n // 6
            char = sdd[sft]
            v2s = 0; v2p = 0; v2t = 0
            if char in CMDS_T0:
            ...
            if v2t != 2:
                ...
                while i2 < ln_n and v2i < len(v2k):
                    ...
                    if ob2_c and ob2_c != "0":
                        lane = 0 if self.v2c else int(ob2_c)
                ...
                while i2 < len(v2o):
                    ...
                    if i2+1 < len(v2o):
                        ...
                        try:
                            lane = 0 if self.v2c else int(ob2_char)
                        except:
                            lane = 8
                        if lane < 8:
            ...
            if self.v2c == 2:
                break
            # Original JS sets v2c = 0 strictly at the end of the step!
            self.v2c = 0
        return notes
    ...
    def parse_url(self, html):
        ...
        for i, p in enumerate(raw_parts):
            ...
            if m_data:
                ...
                # JS split per comma does NOT handle the `+` concat inside Javascript.
                # However, our data is already splitted correctly.
