...
# TextageDecoder (v7) fixing cross-measure v2c and early-exit bugs
class TextageDecoder:
    ...
    def decode_hash_format(self, sdd, ln_n=384):
        ...
        while sft < len(sdd):
            ...
            # "-"(フラグ)が直前にある場合、次のブロックの実行をスキップするJSロジックがある！
            # if(sdd.charAt(sft-1)=="-")continue; は、sftが文字列の終端でなければ
            # JSのロジックでは v2cを1にしたあと "次の文字" をそのまま読まずにループへ戻る
            ...
            if v2c == 2:
                break
            # 重要: v2cはハッシュ定義内でのみ使用され、フラグ処理が終わるとリセットされる
            v2c = 0
        return notes
